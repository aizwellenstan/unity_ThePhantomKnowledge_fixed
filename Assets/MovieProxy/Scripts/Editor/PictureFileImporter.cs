...
public class PictureFileImporter
{
    ...
    [MenuItem("CustomMenu/Register files and create movie proxy")]
    static void RegisterFilesAndCreateMovieProxy()
    {
        importPictureFiles(PictureFileImporterParam.Mode.MovieProxy);
    }
    ...
    [MenuItem("CustomMenu/Import files and create sprite animation")]
    ...
    public static void import(PictureFileImporterParam param)
    {
        ...
        else
        {
            ...
            if ( param.mode == PictureFileImporterParam.Mode.MovieProxy)
            {
                ...
                {
                    Debug.LogError("Must be located under StreamingAssets folder.");
                    return;
                }
            }
            else
            {
                if (dstFolder.StartsWith(Application.dataPath) && dstFolder.StartsWith(Path.Combine(Application.dataPath, "StreamingAssets").Replace("\\", "/")))
                {
                    Debug.LogError("Must not be located under StreamingAssets folder.");
                    return;
                }
            }
